...
function TicketList(props) {
  ...
  // Фильтр пересадок
  const selectedFilter = filterItems.map((el, i) => {
    if (el.isCheck) {
      return i - 1
    }
  ...
  const sortedTickets = (arr) => {
    ...
    if (selectedSort[0].name === 'quick') {
      return arr.sort((a, b) => a.segments[0].duration + a.segments[1].duration > b.segments[0].duration + b.segments[1].duration ? 1 : -1)
    }
  };
